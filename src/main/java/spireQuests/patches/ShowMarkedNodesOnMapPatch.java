...
public class ShowMarkedNodesOnMapPatch {
    ...
    public static class NodeMark {
        ...
        public NodeMark(Texture tex, PowerTip tooltip) {
            render = (sb, x, y, scale) -> {
        ...
        public NodeMark(Array<AtlasRegion> regions, float fps, PowerTip tooltip) {
    ...
    public static class ImageField {
        ...
        public static boolean MarkNode(MapRoomNode node, String questID, Texture texture, PowerTip tooltip){
        ...
        public static boolean MarkNode(MapRoomNode node, String questID, Array<AtlasRegion> regions, float fps, PowerTip tooltip){
            ArrayList<Pair<String, NodeMark>> textures = images.get(node);
            if(textures.size() >= 8){
                LogManager.getLogger().info("Too many markings! Marking was not added");
                return false;
         ...
         */
        public static boolean CheckMarks(MapRoomNode node, String questID){ return CheckMarks(node, questID, (Texture) null); }
         ...
         * @return if a mark with rehions Array<AtlasRegion> made by quest QuestID is found on the node.
         */
        ...
        public static void ClearMark(MapRoomNode node, String questID, Texture texture){
            ArrayList<Pair<String, NodeMark>> textures = images.get(node);
            textures.removeIf(pair -> pair.getKey().equals(questID) && (texture == null || pair.getValue().equalz(texture)));
        ...
        public static void renderImage(MapRoomNode __instance, SpriteBatch sb) {
            ...
            for(Pair<String, NodeMark> pair : imageList) {
                ...
                if (mark != null) {
                    ...
                    mark.render(sb, x + orderOffsetX.get(images)*imgWidth * scale, y + orderOffsetY.get(images)*imgHeight * scale, scale);

                    if (mark.tooltip != null && __instance.hb.hovered) {
                        tips.add(mark.tooltip);
                    
                        float tooltip_offset_x = imgWidth * 2f;
                        float tooltip_offset_y = imgHeight * 1.75f;

                        TipHelper.queuePowerTips(x + tooltip_offset_x, y + tooltip_offset_y, tips);
                    }
